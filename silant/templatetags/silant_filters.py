...
@register.simple_tag(takes_context=True)
def complaint_replace(context, **kwargs):
    return _order_replace(silant.views.ComplaintListView, context, **kwargs)
...
@register.simple_tag(takes_context=True)
def can_add_complaint(context, object_name):
    return _role(context.request.user) in ['MANAGER', 'SERVICE'] and object_name == 'Complaint'
